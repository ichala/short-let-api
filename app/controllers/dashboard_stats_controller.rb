class DashboardStatsController < ApplicationController
  ...
  def generate_reservation_chart(reservations)
    ...
    { labels: %w[Pending Confirmed Refused],
  ...
  def generate_hall_chart(halls)
    ...
    halls.each do |hall|
      ...
      total << hall.reservations.size
      pending << hall.reservations.select { |reservation| reservation.status == 'Pending' }.size
      confirmed << hall.reservations.select { |reservation| reservation.status == 'Confirmed' }.size
    end
    { labels: halls_names,
      data: { total:, pending:, confirmed: } }
  end
  ...
  def generate_recent_stats(reservations, users)
    ...
    { recent_users: users.select(:id, :first_name, :last_name, :email).limit(5),
      recent_reservations: }
  end
end
